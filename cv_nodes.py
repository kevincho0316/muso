...
class ClusterImg:
    ...
    def cluster(self, image, cluster):
        cv_image = tensor2opencv(image, 3)

        data = cv_image.reshape(-1, 3).astype(np.float32)
        criteria = (cv.TERM_CRITERIA_MAX_ITER + cv.TERM_CRITERIA_EPS, 10, 0.001)
        retval, bestLabels, centers = cv.kmeans(data, cluster, None, criteria, 10, cv.KMEANS_RANDOM_CENTERS)

        centers = centers.astype(np.uint8)
        dst = centers[bestLabels]
        # dst = cv.rotate(dst, cv.ROTATE_90_CLOCKWISE).reshape(cv_image.shape)

        return opencv2tensor(dst)
